...
class test_get_json(unittest.TestCase):
    ...
    def test_returns_json_from_path(self):
        self.assertEqual(categorize.get_json(json_path), json)
...
class test_has_text(unittest.TestCase):
    ...
    def test_has_text_ATTACHMENT(self):
        self.assertEqual(categorize.has_text(source_text=json, find_text='ATTACHMENT'),1)
    
    def test_has_text_WEAPON(self):
        self.assertEqual(categorize.has_text(source_text=json, find_text='WEAPON'),True)

    def test_has_text_ATTACHMENT_false(self):
        fail_json = categorize.get_json(main_screen_path)
        self.assertEqual(categorize.has_text(source_text=fail_json, find_text='ATTACHMENT'),False)

    def test_has_text_WEAPON_false(self):
        fail_json = categorize.get_json(main_screen_path)
        self.assertEqual(categorize.has_text(source_text=fail_json, find_text='WEAPON'),True)
...
class test_find_value_by_id(unittest.TestCase):
    ...
    def test_has_text_found_weapon_name(self):
        ...
        self.assertEqual(categorize.find_value_by_id(source_text=pass_json, id=4),'Crystaring')
